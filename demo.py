...
def main():
    ...
    parser.add_argument('--img_folder', type=str, default='example_data/depths', help='Folder with input images')
    ...
    parser.add_argument('--is_depth', dest='is_depth', action='store_true', default=True, help='If set, render all people together also')
    ...
    for img_path in Path(args.img_folder).glob('*.[jp][pn]g'):
        img = np.asarray(Image.open(img_path))[..., None]
        ...
        disparity = np.stack([disparity]*3, axis=-1)
        disparity_color = (disparity - disparity.min()) / (disparity.max() - disparity.min())
        ...
        # det_out = detector(disparity_color)
        ...
        #     boxes = np.array([[0, 0, img_cv2.shape[1], img_cv2.shape[0]]])

        det_out = detector.track(disparity_color)
