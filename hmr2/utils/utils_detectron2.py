...
class DefaultPredictor_Lazy:
    ...
    def __call__(self, original_image):
        ...
        if len(original_image.shape) == 4:
            return self.batch_process(original_image)
        with torch.no_grad():
    ...
    def batch_process(self, images):
        """
        Args:
            images (np.ndarray): an image of shape (B, H, W, C) (in BGR order).
        ...
        with torch.no_grad():
            ...
            images = torch.as_tensor(images.astype("float32").transpose(0, 3, 1, 2))
            inputs = [{"image": img, "height": height, "width": width} for img in images]
