...
class MeshRenderer:
    ...
    def visualize_tensorboard(self, vertices, camera_translation, images, pred_keypoints, gt_keypoints, focal_length=None, nrow=5, padding=2, depths=None):
        images_np = np.transpose(images, (0,2,3,1))
        rend_imgs = []
        ...
        for i in range(vertices.shape[0]):
            ...
            if depths is not None:
                rend_imgs.append(torch.from_numpy(depths[i]))
                nrow = 6
            rend_imgs.append(rend_img)
            rend_imgs.append(rend_img_side)
