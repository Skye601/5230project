...
def get_example(img_path: str|np.ndarray, center_x: float, center_y: float,
    ...
    #################################################depth-anything#################################################
    # image_d = depth_transform({'image': image[:,:,:3]})['image']
    # image_d = torch.from_numpy(image_d).unsqueeze(0).cuda()
    
    # with torch.no_grad():
    #     depth = depth_anything(image_d)
        
    # depth = F.interpolate(depth[None], (patch_width, patch_height), mode='bilinear', align_corners=False)[0, 0]
    # depth = (depth - depth.min()) / (depth.max() - depth.min()) * 255.0
    # depth = depth.cpu().numpy().astype(np.uint8)
    # depth_color = cv2.applyColorMap(depth, cv2.COLORMAP_INFERNO)
    # depth_color = np.concatenate([depth_color, image[:,:,3:]], axis=2)
    # image = depth_color
    #################################################depth-anything#################################################
